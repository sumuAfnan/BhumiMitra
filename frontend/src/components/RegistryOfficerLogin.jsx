...
import { loginRegistryOfficer } from '../api/RegoffApi';
import { useNavigate } from 'react-router-dom';

const RegistryOfficerLogin = () => {
  const [loginData, setLoginData] = useState({
    email: '',
    password: '',
    role: 'Land Registry Officer', // ✅ Role automatically set
  });
 ...
 const handleSubmit = async (e) => {
  ...
  try {
    const res = await loginRegistryOfficer(loginData);
    alert(res.message);

    // ✅ Redirect if login successful
    navigate('/RegistryOfficer');
  } catch (err) {
    alert(err.message);
  }
};
      ...
      <div className="w-full max-w-md bg-white shadow-md rounded-md p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <input
            ...
            readOnly
            className="w-full px-4 py-2 border border-gray-200 rounded-md focus:outline-none bg-green-50 focus:ring-2 focus:ring-lime-400 cursor-not-allowed"
          />
...
export default RegistryOfficerLogin;
